...
## 出力を小さく保つ（こちらのほうが重要）
生成速度が遅いため、1 回の応答が長いとそれだけでタイムアウトします。
実測では出力 1000 トークンで 26 秒、2000 トークンで 53 秒かかります。
**1 回の応答は 500 トークン以内**に収めてください。

- **ファイル全体を書き直さない。** 変更する行だけを置換する
- 新規ファイルも、大きいものは複数回に分けて追記する
- 変更は 1 ファイルずつ。複数ファイルの同時編集はしない
- 手順やコードを長く説明しない。実行してから結果を 2〜3 行で報告する
- 既存コードをそのまま引用し直さない
EOF
