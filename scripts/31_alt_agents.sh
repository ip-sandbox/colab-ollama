...
EOF
  ...
  # --- AGENTS.md（ローカルモデル運用ルール） -----------------------------
  # Codex CLI はワークスペースの git root から AGENTS.md を毎ターン読み込み、
  # プロンプトの先頭に足す（手順書 §5.9・§5.8.1 で実機確認した不具合への対策）。
  # qwen2.5-coder 系は apply_patch の unified diff を安定して組み立てられず、
  # 「〜します」と宣言だけして何も書かない／中身が空のファイルを作る、という
  # 不具合がある。毎回プロンプトで注意する代わりに、ここへ焼き込んでおく。
  mkdir -p "$WORKSPACE"
...
EOF
  ok "AGENTS.md を配置しました: $WORKSPACE/AGENTS.md"
