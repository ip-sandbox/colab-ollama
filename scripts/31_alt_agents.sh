...
EOF
  ...
  # プロンプトの先頭に足す（手順書 §5.9）。
  #
  # ★ 中身はモデルによって変えなければならない。
  #   qwen2.5-coder 系は apply_patch の unified diff を安定して組み立てられず、
  #   「〜します」と宣言だけして何も書かない／中身が空のファイルを作る
  #   （§5.8.1）。その回避策として「apply_patch を使うな、heredoc で全体を
  #   書け」と指示している。
  #   しかし Devstral のように apply_patch 形式の編集で訓練されたモデルに
  #   同じことを言うと、得意な経路をわざわざ塞ぐことになり逆効果になる。
  #   そこで AGENTS_RULESET（プロファイル依存、scripts/common.sh）で切り替える。
  mkdir -p "$WORKSPACE"
  AGENTS_SRC="$(cd "$(dirname "$0")" && pwd)/agents/AGENTS.$AGENTS_RULESET.md"
  [ -f "$AGENTS_SRC" ] || die "AGENTS.md のテンプレートがありません: $AGENTS_SRC
     AGENTS_RULESET=$AGENTS_RULESET は未知の値かもしれません。
     使えるもの: $(ls "$(cd "$(dirname "$0")" && pwd)/agents" 2>/dev/null \
                    | sed 's/^AGENTS\.//; s/\.md$//' | tr '\n' ' ')"
  cp "$AGENTS_SRC" "$WORKSPACE/AGENTS.md"
  ok "AGENTS.md を配置しました (ruleset=$AGENTS_RULESET): $WORKSPACE/AGENTS.md"
