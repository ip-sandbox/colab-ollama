...
# 追記 (2026-09-19): num_ctx の上限を実測 — 律速は VRAM ではなく時間だった

## 環境

- T4（再試行 1 回目で確保）/ `gemma4:12b-it-qat` / Ollama 0.34.2 / q8_0 KV
- **VM は検証後に停止済み（サーバ側の一覧が空であることを確認）**

## 1. VRAM だけ見れば設計上限の 262,144 まで載る

`num_ctx` を変えたモデルを作り、ロードして短い生成まで通した。
**全点で 49/49 層が GPU に載り、CPU への溢れは一度も起きなかった。**

| num_ctx | VRAM 使用 | VRAM 空き | 層オフロード | 判定 |
|---:|---:|---:|---|---|
| 32,768 | 8,025 MiB | 6,888 MiB | 49/49 | OK |
| 65,536 | 8,375 MiB | 6,538 MiB | 49/49 | OK |
| 131,072 | 8,891 MiB | 6,022 MiB | 49/49 | OK |
| 196,608 | 9,627 MiB | 5,286 MiB | 49/49 | OK |
| **262,144** | **10,363 MiB** | **4,550 MiB** | **49/49** | **OK** |

262,144 は gemma4 の設計上限そのもの。**T4 の 15GB に収まってしまった。**

「ロードできた」だけでは上限の確認にならないので、262,144 の構成に
**134,916 トークンの実プロングト**を投入した。450 秒で正常終了、OOM 無し、
49/49 層 GPU のまま、VRAM も 10,369 MiB で安定していた。

### KV の実寸が確定した

傾きは **0.0102 MiB/token**（32,768 → 262,144 で 8,025 → 10,363 MiB）。
Ollama は SWA 層の KV を window=1024 で頭打ちにしている、ということ。

**当初の安全側 0.164 は 16 倍の過大評価だった。** そのせいで事前チェックが
「必要 12,834 MiB」と出したのに実際は 7,681 MiB しか使っていなかった。
`KV_MIB_PER_TOKEN` を **0.03**（実測の約 3 倍、安全側）に改めた。
見積もり 9,426 MiB / 実測 8,375 MiB となり、1GB 程度の余裕を残す妥当な値になった。

## 2. ★ 律速は VRAM ではなく、リクエストの制限時間だった

**prefill 速度は文脈が長くなるほど落ちる。**

| プロンプト長 | prefill |
|---:|---:|
| 13,181 tok | 666 tok/s |
| 134,916 tok | **308 tok/s** |

Cline の 1 リクエスト予算 300 秒（§7 の実測）から、出力 500 トークン分の
31 秒を引くと、prefill に使えるのは 269 秒。各 `num_ctx` を**埋めきる**のに
要する時間は:

| num_ctx | 推定 prefill | 満たすのに | 300 秒予算 |
|---:|---:|---:|---|
| 32,768 | 526 tok/s | 62s | 収まる |
| **65,536** | **419 tok/s** | **156s** | **収まる** |
| 131,072 | 312 tok/s | 419s | ★超過 |
| 196,608 | 250 tok/s | 786s | ★超過 |
| 262,144 | 206 tok/s | 1,274s | ★超過 |

**つまり実用上の上限は 65,536。** それ以上に設定しても、文脈を埋めきる前に
リクエストが切れる。VRAM を余計に食うだけで得が無い。

`_p_ctx` を **32,768 → 65,536** に上げた。VRAM は 8,025 → 8,375 MiB と
350 MiB 増えるだけで、使える文脈が倍になる。

## 3. 停止時に CPU セッションが再生成された（既知の罠を再度踏んだ）

`remote/09_stop.sh` で T4 を止めた直後、`colab sessions` に
**CPU セッションが残っていた**。停止中にまだ走っていた `rsh` が
`colab ssh` 経由でセッションを作り直したもので、RESULT.md §6.4 に
「`colab ssh` は黙って VM を作る」として記録済みの罠である。

`colab stop -s colab-ollama` で停止した（`-s` にはセッション**名**を渡す。
一覧に出る ID を渡すと `not found` になる）。

**教訓: 停止したら必ず `colab sessions` で空を確認する。** `09_stop.sh` の
最後にある確認表示だけでは、その後に再生成された分を捕まえられない。

## 測っていないこと

- **65,536 で実タスクを流していない。** 上限の根拠は prefill 速度からの
  外挿で、Cline/Codex の実タスクで確かめたのは 32,768 のときの 5/5 完走
- prefill 速度の劣化は 2 点（13k と 135k）から対数線形で内挿している。
  中間点は測っていないので、65,536 の 419 tok/s は推定値
- Codex CLI は `stream_idle_timeout_ms` が初回バイトまでを縛らない（§7）ので、
  Cline より長い文脈を使える可能性がある。未確認
